...
//将http请求和响应初始化进控制器基类
func (this *AntController)Init(w http.ResponseWriter,r *http.Request) {
...
//停止
func (this *AntController) StopRun()  {
	panic(ErrAbort)
}

// ParseForm will parse form values to struct via tag.
func (this *AntController)ParseForm(form url.Values, obj interface{}) error {
	...
	return this.parseFormToStruct(form, objT, objV)
}
...
func (this *AntController)parseFormToStruct(form url.Values, objT reflect.Type, objV reflect.Value) error {
	for i := 0; i < objT.NumField(); i++ {
		...
		if fieldT.Anonymous && fieldT.Type.Kind() == reflect.Struct {
			err := this.parseFormToStruct(form, fieldT.Type, fieldV)
			if err != nil {
				return err
			}
			continue
